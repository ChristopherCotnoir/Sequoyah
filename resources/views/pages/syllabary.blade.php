  ...
  <tr>
    ...
    @foreach($vowels as $colIndex => $vowel)
      ...
      <th class='headerCell' for='col-{{{ $colIndex }}}'>
          <b>
              {{{ $vowel['ipa'] }}} <br /> 
              <?php echo '<img src="/syllabary/symbol/$vowel[colIndex][symbol_id]/data"></img>'; ?>
          </b>
      </th>
      <?php $starting_symbol++; ?>
    @endforeach
  ...
  @foreach($consonants as $rowIndex => $consonant)
    <tr>
      ...
      <td class='headerCell' for='row-{{{ $rowIndex }}}'>
          <b>{{{ $consonant['ipa'] }}}</b> <br /> 
          <?php echo '<img src="/syllabary/symbol/$consonant[rowIndex][symbol_id]/data"></img>'; ?>
      </td>
      <?php $starting_symbol++; ?>
      @for($i = 0; $i < count($vowels); $i++)
        <td>
            {{{ $consonant['ipa'] . $vowels[$i]['ipa'] }}} <br /> 
        </td>
        <?php $starting_symbol++; ?>
      @endfor
    </tr>
